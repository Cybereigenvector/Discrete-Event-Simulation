...
/* The structure of a node. This is the basic building block of the  linked list.  */
	...
	//if the event time is same.This ensures that the departure is processed before the 	//
	//arrival when both the events are having the same event time							//
	//////////////////////////////////////////////////////////////////////////////////////////
	void swap(void)
	{
		bool swap=true;
	...
	public:struct node *search(string eventtype,int eventtime,string eventdescription)
	{
	...
	public:void print(void)
	{
		...
		{
			printf("(%s,%d)",HEADPTR->TYPE.c_str(),HEADPTR->TIME);
		}
		else									//If the length of the node is more than 1
		{
			for(CURRENT=HEADPTR;CURRENT!=TAILPTR;CURRENT=CURRENT->NEXT)
	       	{
	  			printf("(%s,%d)",CURRENT->TYPE.c_str(),CURRENT->TIME);
	       	}
	        printf("(%s,%d)",TAILPTR->TYPE.c_str(),TAILPTR->TIME);
		}
	}
