...
/* The structure of a node. This is the basic building block of the  linked list.  */
	...
	struct node *search(string eventtype,int eventtime,string eventdescription)
	{
	...
	public:void print(void)
	{
		...
		{
			printf("(%s,%d,%s)\n",HEADPTR->TYPE.c_str(),HEADPTR->TIME,HEADPTR->DESCRIPTION.c_str());
		}
		else									//If the length of the node is more than 1
		{
			for(CURRENT=HEADPTR;CURRENT!=TAILPTR;CURRENT=CURRENT->NEXT)
				...
				printf("(%s,%d,%s) \n",TAILPTR->TYPE.c_str(),TAILPTR->TIME,TAILPTR->DESCRIPTION.c_str());
		}
	}
	...
	public:struct node *get_next(void)
	{
		...
		else if(HEADPTR==TAILPTR)						//When the list has only one node
		{
			PTR=HEADPTR;
			HEADPTR=TAILPTR=NULL;
			printf("\nGet Next -> (%s,%d,%s) \n",PTR->TYPE.c_str(),PTR->TIME,PTR->DESCRIPTION.c_str());
			return(PTR);
		}
